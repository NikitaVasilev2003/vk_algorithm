...
def build_graph(rows, cols, weights):
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            if r > 0 and weights[current_index - cols] != 0:
                graph[current_index].append(current_index - cols)

            if r < rows - 1 and weights[current_index + cols] != 0:
                graph[current_index].append(current_index + cols)

            if c > 0 and weights[current_index - 1] != 0:
                graph[current_index].append(current_index - 1)

            if c < cols - 1 and weights[current_index + 1] != 0:
                graph[current_index].append(current_index + 1)
    return graph
...
def main():
    try:
        ...
        # Чтение матрицы лабиринта
        weights = []
        for _ in range(rows):
            weights.extend(map(int, input().strip().split()))

        # Чтение координат (стартовые и конечные в одной строке)
        ...
        # Если старт и финиш совпадают, выводим единственную точку
        ...
        # Инициализация структур данных для BFS
